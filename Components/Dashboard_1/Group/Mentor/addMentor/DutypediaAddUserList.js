...
import {
    addMentorDutypediaToMentorList,
    removeMentorFromDutypediaAddlist
} from '../../../../../Redux/Dashboard_1/Action/mentorListAction.js';

const DutypediaAddUserList = (props) => {
    ...
    const searchResult = dutypediaAddList?.filter((search) =>
    ...
    const handleItemAddToMentorList = () => {
        dispatch(addMentorDutypediaToMentorList(selectItem));
        dispatch(removeMentorFromDutypediaAddlist(selectItemId));

        props.setShowMentorList(true);
    };

    const handleAllItemAddToMentorList = () => {
        dispatch(addMentorDutypediaToMentorList(selectItemTwo));
        dispatch(removeMentorFromDutypediaAddlist(newSelectId));

        props.setShowMentorList(true);
    };

    const handleAllSelect = (role, id) => {
        setAllCheck(!allCheck);
    };

    const handelAddStuffPartner = () => {
        setShowStuffPartner(true);
        setSelectItemTwo(dutypediaAddList);
    };
                ...
                {props.showDataForMentor !== undefined ? (
                    ...
                    <div>
                        ...
                        {showStuffPartner === false && (
                            <div>
                                <button className="mentorAdd" disabled mentorAdd>
                                    <i className="">
                                        <FaUserPlus icon={faUserPlus} />
                                    </i>{' '}
                                    Add Member
                ...
                {props.showDataForMentor !== undefined ? (
                    <div>
                        {searchInput !== '' ? (
                            ...
                            <div>
                                {dutypediaAddList.map((addItem, index) => (
                                    <DutypediaAddUserMapList
                                        handleCount={handleCount}
                                        allCheck={allCheck}
                                        addItem={addItem}
                        ...
                        {showStuffPartner === true && (
                            <div>
                                {searchInput !== '' ? (
                                    ...
                                    <div>
                                        {dutypediaAddList.map((addItem, index, array) => (
                                            <DutypediaAddUserMapList
                                                handleCount={handleCount}
                                                allCheck={allCheck}
                                                addItem={addItem}
...
export default DutypediaAddUserList;
