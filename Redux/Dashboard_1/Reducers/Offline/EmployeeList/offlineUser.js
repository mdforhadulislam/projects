...
export const offline_user=(state=initialState,action)=>{
    switch (action.type) {
        ...
        case "DELETE_OFFLINE_USER":{
            return{
                offline_user:state.offline_user.filter(singelUser=>singelUser.user_id!==action.payload)
            }
        }
        case"EDIT_OFFLINE_USER_PROFILE_IMAGE":{
            return{
                offline_user:[
                    ...state.offline_user.filter(user=>user.user_id!==action.payload.id).map(user=>{
                        return{
                            ...user
                        }
                    }),
            ...
            return{
                offline_user:[
                    ...
                    ...state.offline_user.filter(user=>user.user_id===action.payload.id).map(user=>{
                        return{
                            ...user,
                            name:`${action.payload.first_name} ${action.payload.middle_name? action.payload.middle_name:""} ${action.payload.last_name}`
                        }
                    })
                ]
            }
            ...
            return{
                offline_user:[
                    ...
                    ...state.offline_user.filter(user=>user.user_id===action.payload.id).map(user=>{
                        return{
                            ...user,
                            user_documentes:[...user.user_documentes.filter(singel_documents=>singel_documents.id!==action.payload.value).map(documentes=>{
                                return{
                                    ...documentes
                                }
                            })]
