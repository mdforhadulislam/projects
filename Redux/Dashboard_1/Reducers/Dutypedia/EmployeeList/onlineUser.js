...
export default function create_online_user(state = initialState, action) {
    switch (action.type) {
        ...
        case "USER_ACCESS_FUNCTIONALY_EDIT": {
            return {
                user: [
                    ...state.user.filter((user) => { return (user.user_id !== action.payload.id) }).map((user) => {
                        return {
                            ...user,
                        }
                    }),
                    ...state.user.filter((user) => { return (user.user_id === action.payload.id) }).map((user) => {
                        return {
                            ...
                            accessFunctionality: {
                                ...
                                user_no_salary_system: user.accessFunctionality.user_no_salary_system
                            }
                        }
                    })
                ]
            ...
            return {
                user: [
                    ...
                    ...state.user.filter(user => { return (user.user_id === action.payload.id) }).map(user => {
                        return {
                            ...user,
                            user_documentes: [...user.user_documentes.filter(file => file.id !== action.payload.fileId)]
                        }
                    })
                ]
            }
                ...
                user: [
                    ...
                    ...state.user.filter(user => { return (user.user_id === action.payload.id) }).map(user => {
                        return {
                            ...
                            user_salary: {
                                ...user.user_salary,
                                salary_status: action.payload.salary_status
                            }
                        }
                    })
                ]
